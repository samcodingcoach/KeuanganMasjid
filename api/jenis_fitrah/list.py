...
def get_jenis_fitrah_list(supabase_client):
    """Get all jenis_fitrah data ordered by created_at (newest first) with GMT+8 formatting"""
    try:
        # Get data from supabase, ordered by created_at descending (newest first)
        response = supabase_client.table('jenis_fitrah').select(
            'id_jenis_fitrah, created_at, nama_jenis, aktif'
        ...
        if response.data:
            ...
            return jsonify({
                'success': True,
                'message': 'No jenis_fitrah data found',
                'data': []
            })
