...
fetch('/api/transaksi.list')
    ...
    .then(response => {
        if (response.success && response.data) {
            // Filter transactions to only show expense transactions (kode_transaksi starting with 'FS')
            allExpenseTransactions = response.data.filter(transaction =>
                transaction.kode_transaksi && transaction.kode_transaksi.startsWith('FS')
            );

            // Initialize the table with all transactions
            filteredTransactions = allExpenseTransactions;
            renderTable();
        } else {
...
function renderTable() {
    ...
    if (filteredTransactions.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="text-center py-3">Tidak ada data pengeluaran</td></tr>';
        document.getElementById('total-pengeluaran-td').textContent = 'Rp 0';
    } else {
        filteredTransactions.forEach((transaction, index) => {
            ...
            totalPengeluaran += parseInt(transaction.total || 0);
            ...
            const addButtonHtml = isClosed ? '' : `
                <button class="btn btn-sm btn-outline-whatsapp p-1 me-1" onclick="addExpenseDetail(${JSON.stringify(transaction).replace(/"/g, '&quot;')})" style="border-radius: 8px; width: 36px; height: 36px;" title="Tambah Detail">
        ...
        // Format total pengeluaran and display it
        document.getElementById('total-pengeluaran-td').textContent = 'Rp ' + totalPengeluaran.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
    }
...
function filterTransactions() {
    ...
    filteredTransactions = allExpenseTransactions.filter(transaction => {
...
function showDetail(transaction) {
    ...
    document.getElementById('detail_nama_mustahik').textContent = transaction.nama_mustahik || '-';
...
function loadEditCategories(currentCategoryId) {
    fetch('/api/kategori.list')
        ...
        .then(response => {
            if (response.success && response.data) {
                const selectElement = document.getElementById('edit_kategori_pengeluaran');
                selectElement.innerHTML = '<option value="">Pilih Kategori Pengeluaran</option>'; // Clear existing options

                response.data.forEach(category => {
                    if (category.jenis_kategori === 'Pengeluaran') { // Only show expense categories
                ...
                setTimeout(function() {
                    if ($('#edit_kategori_pengeluaran').data('select2')) {
                        $('#edit_kategori_pengeluaran').select2('destroy');
                    }
                    $('#edit_kategori_pengeluaran').select2({
                        theme: 'bootstrap-5',
...
// Function to generate kode transaksi in format FSXXX-YYMMDD
function generateKodeTransaksi(transactions, selectedDate = null) {
    ...
    const prefix = 'FS'; // Fixed prefix for expense transactions
    ...
    todayTransactions.forEach(transaction => {
        const match = transaction.kode_transaksi.match(/^FS(\d{3})-\d{6}$/);
        if (match) {
    ...
    // Generate the next number (odd numbers sequence: 001, 003, 005, etc.)
    let nextNumber;
    if (maxNumber === 0) {
        nextNumber = 1;  // Start with 001 for first transaction of the day
    } else if (maxNumber % 2 === 0) {
        // If maxNumber is even, the next odd number is maxNumber + 1
        nextNumber = maxNumber + 1;
    } else {
        // If maxNumber is odd, the next odd number is maxNumber + 2
        nextNumber = maxNumber + 2;
    }

    return `${prefix}${nextNumber.toString().padStart(3, '0')}-${datePart}`;
}
...
function validateDateInput() {
    ...
    if (allExpenseTransactions) {
        const newKode = generateKodeTransaksi(allExpenseTransactions, selectedDateStr);
...
document.getElementById('addTransactionModal').addEventListener('shown.bs.modal', function() {
    ...
    const newKode = generateKodeTransaksi(allExpenseTransactions || [], dateStr);
    document.getElementById('kode_transaksi').value = newKode;

    // Load accounts and mustahik when modal opens to ensure data is fresh
    loadAccounts();
    loadMustahik();
...
function loadMustahik() {
    fetch('/api/mustahik.list')
        ...
        .then(response => {
            if (response.success && response.data) {
                const selectElement = document.getElementById('nama_mustahik');
                selectElement.innerHTML = '<option value="">Pilih Nama Mustahik</option>'; // Clear existing options

                response.data.forEach(mustahik => {
                    const option = document.createElement('option');
                    option.value = mustahik.id_mustahik;
                    option.text = mustahik.nama_lengkap + ' [' + mustahik.kategori + ']';
                    selectElement.appendChild(option);
                });
                ...
                // Only initialize or refresh if modal is currently shown
                if ($('#addTransactionModal').hasClass('show')) {
                    if ($('#nama_mustahik').data('select2')) {
                        $('#nama_mustahik').select2('destroy').select2({
                            theme: 'bootstrap-5',
                ...
                console.error('API mustahik.list returned error:', response);
            }
        })
        .catch(error => {
...
// Load mustahik when DOM is ready (accounts are loaded when modal is opened)
document.addEventListener('DOMContentLoaded', function() {
    loadMustahik();
});
...
// Function to open the add expense detail modal
function addExpenseDetail(transaction) {
    ...
    // Show the modal
    const modal = new bootstrap.Modal(document.getElementById('addExpenseDetailModal'));
    modal.show();
}
...
function loadCategories() {
    fetch('/api/kategori.list')
        ...
        .then(response => {
            if (response.success && response.data) {
                const selectElement = document.getElementById('kategori_pengeluaran');
                selectElement.innerHTML = '<option value="">Pilih Kategori Pengeluaran</option>'; // Clear existing options
...
document.getElementById('addExpenseDetailModal').addEventListener('shown.bs.modal', function() {
    ...
    // Initialize Select2 for searchable dropdowns when modal is shown
    setTimeout(function() {
        $('#kategori_pengeluaran').select2({
            theme: 'bootstrap-5',
            placeholder: 'Pilih Kategori Pengeluaran',
            allowClear: true,
            width: '100%',
            dropdownParent: $('#addExpenseDetailModal')
        });
    }, 100);
});
...
// Function to save new expense detail
document.getElementById('save-expense-detail-btn').addEventListener('click', async function() {
    ...
    const id_kategori = document.getElementById('kategori_pengeluaran').value;
    ...
    if (bukti) {
        ...
        const transaction = allExpenseTransactions.find(t => t.id_transaksi == id_transaksi);
        if (!transaction) {
    ...
    .then(data => {
        if (data.success) {
            showTransactionToast('Detail pengeluaran berhasil disimpan');

            // Close the modal
            const modal = bootstrap.Modal.getInstance(document.getElementById('addExpenseDetailModal'));
            modal.hide();
            ...
            showTransactionToast('Gagal menyimpan detail pengeluaran: ' + (data.message || data.error), 'error');
        }
    })
    .catch(error => {
        console.error('Error saving expense detail:', error);
        showTransactionToast('Terjadi kesalahan saat menyimpan detail pengeluaran', 'error');
    });
});
...
document.getElementById('save-edit-detail-btn').addEventListener('click', async function() {
    ...
    const id_kategori = document.getElementById('edit_kategori_pengeluaran').value;
    ...
    .then(data => {
        if (data.success) {
            showTransactionToast('Detail pengeluaran berhasil diperbarui');
            ...
            fetch('/api/transaksi.list')
                ...
                .then(response => {
                    if (response.success && response.data) {
                        ...
                        // Find the current transaction in allExpenseTransactions to get updated details
                        const currentTransaction = allExpenseTransactions.find(t => t.id_transaksi == currentTransactionId);
            ...
            showTransactionToast('Gagal memperbarui detail pengeluaran: ' + (data.message || data.error), 'error');
        }
    })
    .catch(error => {
        console.error('Error updating expense detail:', error);
        showTransactionToast('Terjadi kesalahan saat memperbarui detail pengeluaran', 'error');
    });
});
...
document.getElementById('save-transaction-btn').addEventListener('click', function() {
    ...
    const id_mustahik = document.getElementById('nama_mustahik').value;
    const kode_transaksi = document.getElementById('kode_transaksi').value;

    // Validate required fields
    if (!tanggal_transaksi || !id_akun || !id_mustahik || !kode_transaksi) {
        showTransactionToast('Harap lengkapi semua data yang diperlukan', 'error');
        return;
    }
    ...
    const requestData = {
        ...
        id_muzakki: null,  // Set to NULL as specified for expense transactions
            ...
            fetch('/api/transaksi.list')
                ...
                .then(response => {
                    if (response.success && response.data) {
                        ...
                        const newKode = generateKodeTransaksi(allExpenseTransactions);
                        document.getElementById('kode_transaksi').value = newKode;
                    }
                })
                .catch(error => {
